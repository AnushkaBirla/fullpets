import {data} from './databases/petsDB'
...
router.get('/petData', function(req, res, next) {
    ...
    // later either use a foreach or .map to b cleaner
    // var searchResult = data.map(searchFunction)
    ...
    for (i = 0; i < data.length; i++) {
        if (data[i].breed === req.query.breed && data[i].animal === req.query.animal && data[i].age <= req.query.maxage) {
